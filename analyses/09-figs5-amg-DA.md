...
- [<span class="toc-section-number">1</span> Tasks](#tasks)
  ...
  - [<span class="toc-section-number">1.2</span> Export Figure S5 as
    TIFF](#export-figure-s5-as-tiff)
...
# Read deduplicated pathway table from 04-fig4-heatmap
dedup_path <- path_source(
  "04-fig4-heatmap",
  "deduplicated_pathway_table.xlsx"
)
...
if (!file.exists(dedup_path)) {
  stop(
    paste(
      "deduplicated_pathway_table.xlsx not found in",
      "04-fig4-heatmap outputs"
    )
  )
}
...
``` r
# Special gene name mapping (same as Fig4b)
...
![](09-figs5-amg-DA_files/figure-commonmark/plot-amg-heatmap-da-1.png)
...
``` r
# Save figures
ggsave(path_target("FigS5_top20_AMG_DA_heatmap.png"),
       plot = p_amg_da, width = 8, height = 10, dpi = 300)
ggsave(path_target("FigS5_top20_AMG_DA_heatmap.pdf"),
...
message(
  "✅ FigS5 completed: ",
  nrow(gene_log_matrix_da),
  " candidate AMG annotations"
)
```

</details>

    ✅ FigS5 completed: 20 candidate AMG annotations

### Export Figure S5 as TIFF
