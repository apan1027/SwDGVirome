...
```{r}
#| label: export-fig3-tiff
#| include: false
#| message: true
#| warning: false

# Create one folder for all Figure 3 TIFF panels
...
# Export ggplot/patchwork objects as 600-dpi TIFF
save_tiff <- function(plot, filename, width, height) {
  ggsave(
    filename = file.path(tiff_dir, filename),
    plot = plot,
    device = "tiff",
    width = width,
    height = height,
...
# Fig. 3a
stopifnot(exists("p_fig3a"))

save_tiff(
  plot = p_fig3a,
  filename = "Fig3a_abundant_abundance.tiff",
  width = 4.5,
  height = 4
)

# Fig. 3b
stopifnot(exists("p_fig3b"))

save_tiff(
  plot = p_fig3b,
  filename = "Fig3b_abundant_lifestyle.tiff",
  width = 4.5,
  height = 4
)

# Fig. 3c
stopifnot(exists("venn_plot_abundant"))

grDevices::tiff(
  filename = file.path(
    tiff_dir,
    "Fig3c_abundant_venn.tiff"
  ),
  width = 2000 / 300,
  height = 2000 / 300,
  units = "in",
...
grid::grid.newpage()
grid::grid.draw(venn_plot_abundant)
grDevices::dev.off()

# Fig. 3d
stopifnot(exists("p_fig3d"))

save_tiff(
  plot = p_fig3d,
  filename = "Fig3d_rare_abundance.tiff",
  width = 4.5,
  height = 4
)

# Fig. 3e
stopifnot(exists("p_fig3e"))

save_tiff(
  plot = p_fig3e,
  filename = "Fig3e_rare_lifestyle.tiff",
  width = 4.5,
  height = 4
)

# Fig. 3f
stopifnot(exists("venn_plot_rare"))

grDevices::tiff(
  filename = file.path(
    tiff_dir,
    "Fig3f_rare_venn.tiff"
  ),
  width = 2000 / 300,
  height = 2000 / 300,
  units = "in",
...
grid::grid.newpage()
grid::grid.draw(venn_plot_rare)
grDevices::dev.off()

message(
  "Figure 3 TIFF panels exported to: ",
